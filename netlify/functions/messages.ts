...
export const handler: Handler = async (event) => {
  ...
  if (method === "POST" && path.includes("/transaction/mark-complete")) {
    ...
    try {
      ...
      const thread = threadResult.rows[0];
      console.log("[mark-complete] Thread found:", {
        threadId,
        participants: thread.participants,
        transaction: thread.transaction,
      });
      ...
      console.log("[mark-complete] Parsed:", {
        tx,
        participants,
        participantsType: typeof thread.participants,
      });
      ...
      if (
        ...
        console.log("[mark-complete] FAILED: Invalid participants", {
          participants,
      ...
      console.log("[mark-complete] Actor check:", {
        userId,
      ...
      console.log("[mark-complete] Transaction state:", {
        currentStatus: tx.status,
        markedCompleteBy: tx.markedCompleteBy,
        userId,
      });
      ...
      if (!tx.markedCompleteBy) {
        ...
        console.log(
          "[mark-complete] First mark - moving to pending_confirmation",
        );
      } else if (tx.markedCompleteBy !== userId) {
        // Second mark by the other party: complete transaction
        newStatus = "completed";
        console.log(
          "[mark-complete] Second mark by other party - completing transaction",
        );
      } else {
        // Same user trying to mark twice
        console.log(
          "[mark-complete] Same user trying to mark twice - returning 409",
        );
        return {
          statusCode: 409,
          headers: { "Content-Type": "application/json" },
  ...
  if (method === "POST" && path.includes("/transaction/agree")) {
    ...
    try {
      ...
      // Fetch partner data - parse participants if needed
      const threadParticipants =
        typeof updatedThread.participants === "string"
          ? JSON.parse(updatedThread.participants)
          : updatedThread.participants;
      const partnerId = threadParticipants.find((p: string) => p !== userId);
      const partnerResult = partnerId
        ? await client.query(
            "SELECT id, username, avatar_url, dow_verified_at FROM users WHERE id = $1",
            [partnerId],
    ...
    try {
      ...
      await client.query(
        ...
        [
          ...
          threadId,
          null,
          `Dispute opened: ${body.reason || "No reason provided"}`,
          now,
          "system",
        ],
      );
